...
> **SSoT update:** este documento queda como input histórico/reference-only. La fuente de verdad backend vigente es `docs/spec-backend-ssot.md` y los cambios `openspec/changes/backend-*` listados ahí. Si hay conflicto, ganan `openspec/config.yaml` y las nuevas slices.
...
- [ ] `coordinatorRespond(id,'reasignar', new_person_id)` _(nice-to-have)_: re-propone al nuevo candidato.
